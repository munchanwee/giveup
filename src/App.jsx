...
export default function App() {
  ...
  const result = results[resultData.key] || results['cut-sense-close-quick'];
    ...
    return (
      <main className="page intro-page">
        ...
        <section className="hero frame">
          <div className="intro-mark">QUIT SENSE CHECK</div>
          <h1>포기력 지수</h1>
          <p className="subtitle">나는 무엇을 어떻게 그만두는가.</p>
          <p className="note highlight">포기는 실패가 아니라, 더 이상 들고 가지 않기로 한 것을 알아차리는 감각일 수 있습니다.</p>
          <p className="action-note">이 검사는 런닝 능력을 보는 검사가 아닙니다.<br />작은 결심이 시작되고, 흔들리고, 멀어지는 과정을 따라갑니다.</p>
          <div className="product-spec" aria-label="검사 정보">
            <span>16 SCENES</span>
            <span>ABOUT 2 MIN</span>
            <span>NO SCORE</span>
          </div>
          <button className="primary-btn" onClick={start}>검사 시작하기 →</button>
          <DecorativeCharacter src={characters.duo} className="intro-character" alt="위위와 문문이" />
          <p className="result-note">점수는 나오지 않습니다.<br />결과는 하나의 포기 상태값으로 출력됩니다.</p>
          <div className="intro-copy">
            <p>이 점검은 좋고 나쁨을 판단하지 않습니다.</p>
            <p>시작한 일이 어떻게 흐려지는지,<br />어디서 멈추고, 무엇을 남기고,<br />어떤 방식으로 닫히는지 살펴봅니다.</p>
          </div>
        </section>
      </main>
    );
    ...
    return (
      <main className="page result-page">
        ...
        <section className="frame result-card">
          <p className="eyebrow">{`RESULT_${String(result.id).padStart(2, '0')} · 포기력 지수 결과`}</p>
          ...
          <details className="debug-panel">
            ...
            <p>방식: {axisLabels[resultData.selected.style]} / 기준: {axisLabels[resultData.selected.criterion]} / 처리: {axisLabels[resultData.selected.closure]} / 속도: {axisLabels[resultData.selected.speed]}</p>
          </details>
      ...
      <section className="frame question-card">
        ...
        <div className="button-row nav-row">
          ...
          <button className="primary-btn small" onClick={next} disabled={!answers[question.id]}>
            {current === questions.length - 1 ? '포기력 확인하기' : '다음 장면으로'}
          </button>
        </div>
      </section>
    </main>
